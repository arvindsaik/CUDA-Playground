...
unsigned int* wbImport(const char* fName, int* numElements)
{
    ...
    *numElements = 0;
    ...
    unsigned int * UBuf = (unsigned int *) malloc(*numElements * sizeof(unsigned int));
    ...
    std::copy(UVec.begin(), UVec.end(), UBuf);

    return UBuf;
}
//newly added for output reading

unsigned int* wbImportOutput(const char* fName, int* numElements)
{
    std::ifstream inFile(fName);
    ...
    *numElements = 0;
    while (inFile >> a)
    {
    	// cout<<*numElements<<" : "<<(char)*numElements<<" : "<<a<<"\n";
        UVec.push_back(a);
        *numElements += 1;
    }
...
{
    int solnItems;
    unsigned int * soln = wbImportOutput(wbArg_getInputFile(args, args.argc - 3), &solnItems);
